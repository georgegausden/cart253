...
let userCannons = 0;
let maxCannons = 5;
let numberOfCannonsAvailable = undefined;
...
function setup() {
  ...
  //set the initial amount of lives for the user and cannons
  lives = livesi;
  numberOfCannonsAvailable = maxCannons;
}
...
function instructions() {
  ...
  let backToMenu = createButtonVariable(width / 2, (height / 2 + 150), 200, 100, "Back to menu", 25);
...
function mouseClicked() {
  if (cannons.length < maxCannons){
    cannons.push(createCannonVariable(userCannons));
    userCannons+=1;
    numberOfCannonsAvailable -= 1;
  }
...
//function to display the number of cannons the user has left to shoot
function displayUserCannons(){
  //display the number of lives in the top right corner
  let spacing = 50;
  let displayCannons = numberOfCannonsAvailable;
  let yposition = height / 2 - 190;
  let xposition = width / 2 - 250;
  let imageSize = 50;

  while (displayCannons > 0) {
    //keep count of how many lives are left as heart images
    imageMode(CENTER);
    image(cannonball, xposition + displayCannons * spacing, yposition, imageSize, imageSize);
    displayCannons -= 1;
  }
}
